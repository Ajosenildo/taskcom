...
serve(async (req) => {
  ...
  try {
    ...
    if (!empresa_id) throw new Error("Administrador não associado a uma empresa.");
    ...
    if (profileError) {
      await supabaseAdmin.auth.admin.deleteUser(newAuthUserId); // Desfaz a criação se o perfil falhar
      throw profileError;
    }
